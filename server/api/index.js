...
// ========== Search API (HTTP fallback for WebSocket) ==========
...
app.post('/bookSourceDebug', (req, res) => {
  const { tag, key } = req.body;
...
app.post('/rssSourceDebug', (req, res) => {
  const { tag, key } = req.body;
...
app.get('/', (req, res) => {
  res.json({ 
    status: 'Legado Server is running', 
    version: '1.0.0',
    endpoints: [
      '/getBookshelf',
      '/saveBook',
